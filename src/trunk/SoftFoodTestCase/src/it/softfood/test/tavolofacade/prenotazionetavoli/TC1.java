...
public class TC1 extends TestCase {
	...
	public void setUp() throws Exception {
		...
		tavoliInseriti.add(tavoloInserito.getRiferimento());
	...
	public void testOccupaTavoli() {
		...
		try {
			...
			try {
				tavoloFacade.occupaTavoli(user1, tavoliInseriti);
			} catch (TavoloOccupatoException e) {
				fail("TavoloOccupatoException");
			}
		...
		Boolean stato = false;
		ArrayList<Tavolo> tavoli = null;
		try {
			tavoli = (ArrayList<Tavolo>) tavoloFacade.selezionaTavoliOccupati(user);
		} catch (RemoteException e) {
			fail("RemoteException");
		}
		
		for (Tavolo tavolo : tavoli) {
			if (tavolo.getRiferimento().equals("Tavolo Test 1+Tavolo Test 2")) {
				stato = true;
				break;
			}
		}
