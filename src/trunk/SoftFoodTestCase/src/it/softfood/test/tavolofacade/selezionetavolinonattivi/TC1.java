...
public class TC1 extends TestCase {
	...
	public void setUp() throws Exception {
		...
		tavoloInserito.setAttivo(false);
	...
	@Test
	public void testSelezionaTavoliNonAttivi() {
		ArrayList<Tavolo> tavoliAttuali = null;
		User user1 = null;
		try {
			user1 = userFacade.login(Ruolo.CAMERIERE, "1234");
			tavoliAttuali = (ArrayList<Tavolo>) tavoloFacade.selezionaTavoliNonAttivi(user1);
		...
		Boolean stato = false;
		for (Tavolo tavolo : tavoliAttuali) {
			if (tavolo.equals(tavoloInserito)) {
				stato = true;
				break;
			}
		}
